...
export default class extends Controller {
  ...
  async initializePaymentElement() {
    try {
      ...
      const response = await fetch(
        `/${this.accountSlugValue}/payments/${this.paymentIdValue}/create_setup_intent_for_resolve_payment?token=${this.resolveTokenValue}`,
        {
          method: "POST",
          headers: {
  ...
  async handleSubmit(event) {
    ...
    try {
      // First, get the payment method from the elements
      ...
      if (setupError) {
        ...
        this.showError(setupError.message);
        return;
      }

      // If we have a setup intent, send it to our server via POST
      if (setupIntent) {
        // Send the setup intent to our server via POST
        const response = await fetch(
          `/${this.accountSlugValue}/payments/${this.paymentIdValue}/retry_payment?token=${this.resolveTokenValue}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        // If no setup intent, redirect to the return URL (this is a fallback)
        window.location.href = `/${this.accountSlugValue}/payments/${this.paymentIdValue}/retry_payment?token=${this.resolveTokenValue}`;
      }
    } catch (error) {
      this.toggleButtonState(false);
      this.showError(this.errorMessageValue || "An unexpected error occurred.");
